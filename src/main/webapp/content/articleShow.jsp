				...
				<div class="row">
					<div class="col-xs-2 col-md-1">
						<a href="${APP_PATH}/userController/getOthers?userid=${article_Show.userid }"> 
							...
							<c:if test="${article_Show.userphoto!=null }">
								<img class="img_head" alt="Brand" src="${basepath }/userimg/${article_Show.userphoto }">
							</c:if>
						</a>
					</div>
				...
				<div class="row">
					<div class="col-md-12">
						<c:if test="${article_Show.photo!=\"photo\" }">
							<a href="${basepath }/userarticle/${article_Show.photo }" target="_blank">
								<c:if test="${article_Show.photo.endsWith(\".mp4\")||article_Show.photo.endsWith(\".avi\") }">
									<video class="img_content" controls="controls" src="${basepath }/userarticle/${article_Show.photo }"></video>
								</c:if>
								<c:if test="${!article_Show.photo.endsWith(\".mp4\")&&!article_Show.photo.endsWith(\".avi\") }">
									<img class="img_content" src="${basepath }/userarticle/${article_Show.photo }">
								</c:if>
							</a>
						</c:if>
					</div>
