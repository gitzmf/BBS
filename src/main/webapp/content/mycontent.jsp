					...
					<div class="col-md-2">
						...
						<c:if test="${myListUser.userphoto!=null }">
							<%-- <img class="img-thumbnail"
								...
								<img class="img-thumbnail"
								style="position: relative; width: 140px; height: 140px; left: 10px; top: -20px;"
								src="${basepath }/userimg/${myListUser.userphoto }"> 
						</c:if>
						</a>
					</div>
					<div class="col-md-12 col-xs-12">
						...
						<div class="row">
							<div class="col-md-10 col-xs-7">
								<b>性别年龄</b> &nbsp;&nbsp;&nbsp; 
								<c:if test="${myListUser.sex == null and myListUser.age == null} ">
									<span></span>
								</c:if>
								 <c:if test="${myListUser.sex != null and myListUser.age != null }">
									<span>${myListUser.sex }、${myListUser.age }</span>
								</c:if>
								<c:if test="${myListUser.sex == null and myListUser.age != null }">
									<span>无、${myListUser.age }</span>
								</c:if>
								<c:if test="${myListUser.sex != null and myListUser.age == null }">
									<span>${myListUser.sex }、无</span>
								</c:if> 
								<%-- <span>${myListUser.sex }、${myListUser.age }</span> --%>
							</div>
							...
							<c:forEach var="myListArticle" items="${myListArticles }">
								...
								<div class="row">
									...
									<div class="col-xs-12 col-md-12">
										...
										<c:if test="${myListArticle.photo!=\"photo\" }">
											<a href="${basepath }/userarticle/${myListArticle.photo }" target="_blank">
												<c:if test="${myListArticle.photo.endsWith(\".mp4\")||myListArticle.photo.endsWith(\".avi\") }">
													<video controls="controls" src="${basepath }/userarticle/${myListArticle.photo }"
														style="position: relative; width: 30%; height: 30%;"></video>
												</c:if>
												<c:if test="${!myListArticle.photo.endsWith(\".mp4\")&&!myListArticle.photo.endsWith(\".avi\") }">
													<img src="${basepath }/userarticle/${myListArticle.photo }"
														style="position: relative; width: 30%; height: 30%;">
												</c:if>
											</a>
										</c:if>
									...
									<div class="row">
										...
										<div class="col-xs-12 col-md-12">
											<c:if test="${map_huifu[myArticle_Fid].photo!=\"photo\" }">
												<a href="${basepath }/userarticle/${map_huifu[myArticle_Fid].photo }" target="_blank">
													<c:if test="${map_huifu[myArticle_Fid].photo.endsWith(\".mp4\")||map_huifu[myArticle_Fid].photo.endsWith(\".avi\") }">
														<video controls="controls" src="${basepath }/userarticle/${map_huifu[myArticle_Fid].photo }"
															style="position: relative; width: 30%; height: 30%;"></video>
													</c:if>
													<c:if test="${!map_huifu[myArticle_Fid].photo.endsWith(\".mp4\")&&!map_huifu[myArticle_Fid].photo.endsWith(\".avi\") }">
														<img src="${basepath }/userarticle/${map_huifu[myArticle_Fid].photo }"
															style="position: relative; width: 30%; height: 30%;">
													</c:if>
												</a>
											</c:if>
													...
													<a href="${APP_PATH}/userController/getOthers?userid=${myListAttention.userid }">
														...
														<c:if test="${myListAttention.userphoto!=null }">
															<img class="img-thumbnail" alt="Brand" style="position:relative;width: 70px;height: 70px;"
																src="${basepath }/userimg/${myListAttention.userphoto }">
														</c:if>
														<!-- 该用户没有修改头像，只有默认头像 -->
														<c:if test="${myListAttention.userphoto==null }">
															<img class="img-thumbnail" alt="Brand" style="position:relative;width: 70px;height: 70px;"
												...
												<div class="col-xs-3 col-md-2">
													<a href="${APP_PATH}/userController/getOthers?userid=${myListAttention_be.userid }">
														...
														<c:if test="${myListAttention_be.userphoto!=null }">
															<img class="img-thumbnail" alt="Brand" style="position:relative;width: 70px;height: 70px;"
																src="${basepath }/userimg/${myListAttention_be.userphoto }">
														</c:if>
														<!-- 该用户没有修改头像，只有默认头像 -->
														<c:if test="${myListAttention_be.userphoto==null }">
															<img class="img-thumbnail" alt="Brand" style="position:relative;width: 70px;height: 70px;"
